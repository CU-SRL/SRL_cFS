...
/*                                                                 */
/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */   
bool CFS_IsPacketFiltered(CFE_MSG_Message_t* MessagePtr,
                             uint16          FilterType,
                             uint16          Algorithm_N,
                             uint16          Algorithm_X,
                             uint16          Algorithm_O)
    ...
    */ 
    ...
    else
    {
        if (FilterType == CFS_PKT_SEQUENCE_BASED_FILTER_TYPE)
        {
            ...
            */ 
            PacketValue = CCSDS_RD_SEQ(MessagePtr->CCSDS.Pri);
        }
        else
        {
            /* 
	    ...
	    CFE_TIME_SysTime_t TimeFromMsg = {0};
    	    CFE_MSG_GetMsgTime(MessagePtr, &TimeFromMsg);	    
            PacketTime = TimeFromMsg;  
